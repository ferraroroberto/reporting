...
def load_database_list():
    ...
    try:
        ...
        log.info("Loaded %d database ID to table mappings", len(id_to_table_map))
        return id_to_table_map

    except Exception as e:
        log.error("Error loading database list: %s", e)
        return {}
...
def main():
    ...
    log.info("Extracting relationships from database structure files...")
    ...
    if not samples_dir.exists():
        log.error("Database samples directory not found: %s", samples_dir)
        return
    ...
    log.info("Found %d structure files", len(structure_files))
    ...
    for structure_file in structure_files:
        try:
            ...
            if not database_id:
                log.warning("Could not extract database ID from filename: %s", filename)
                continue

            log.info("Processing: %s", database_id)
            ...
            if relationship_fields:
                log.info("  Found %d relationship fields", len(relationship_fields))
                ...
                log.warning("  No relationship fields found")

        except Exception as e:
            log.error("Error processing %s: %s", structure_file.name, e)
            continue
    ...
    log.info("Relationship extraction completed!")
    log.info("Processed %d databases", len(all_relations))
    log.info("Results saved to: %s", output_file.name)

    # Show example output
    if all_relations:
        log.info("Example output format:\n%s", json.dumps(all_relations[0], indent=2))
