...
This file was moved from the root directory's init.py to better organize
the codebase and separate orchestration logic from entry points.
"""
...
# Add the parent directory to the Python path to access other modules
...
def configure_logger(debug_mode=False):
    ...
    logger = setup_logger("pipeline", file_logging=False, level=log_level)
    return logger
...
def run_pipeline(debug_mode=False, skip_api=False, skip_processing=False, 
    ...
    Args:
        ...
        skip_api: Skip the API data collection step
        skip_processing: Skip the data processing step
        skip_aggregation: Skip the profile aggregation step
        skip_consolidation: Skip the posts consolidation step
        skip_notion: Skip the Notion update step
        reference_date: Reference date in YYYYMMDD format for processing
    """
