...
def get_linkedin_posts(config):
    """Fetch LinkedIn posts data using the API."""
    if not config or 'linkedin_posts' not in config:
        print("LinkedIn posts configuration not found in config file")
        return None
    
    linkedin_config = config['linkedin_posts']
...
def save_results(data):
    ...
    filename = f"linkedin_posts_{current_date}.json"
...
def main():
    """Main function to execute the LinkedIn posts retrieval."""
    # Load configuration
    config = load_config()
    if not config:
        return
    ...
    posts_data = get_linkedin_posts(config)
    if not posts_data:
        return
